...
- Read [Beginner Tutorial](/docs/guides/beginner-tutorial) if the tuple style is still new and you want one feature built slowly.
- Read [Simple Recipes](/docs/recipes/simple-recipes) when you want small, single-feature snippets.
- Read [Read IDs and Fetch Records](/docs/recipes/read-ids-fetch-records) for a concrete `forEach()` plus `fetchJsonSafe()` workflow.
- Read [Examples](/docs/guides/examples) when you want copy-pasteable small patterns and a few larger compositions.
