...
```ts
type TupleRouteHandler<Context, Data, ErrorType = Error> = (
  context: Context,
...
- Success tuples become `context.json(data, status)`
- `HttpError` instances map to their configured status code
- unknown errors become a generic `500`
- `options.mapError()` lets you normalize custom domain errors into `HttpError`

## When this module is enough

If your framework only needs a `json()` method on the context, this adapter is
usually enough. If you need framework-specific middleware composition, keep that
outside Yieldless and let `honoHandler()` sit at the last step.
