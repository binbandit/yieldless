...
This recipe shows the shape Yieldless is best at: an HTTP request that validates
input, performs a few pieces of I/O, retries the flaky part, and returns a
...
<ul class="yl-checklist">
  <li>Validate early and return early.</li>
  <li>Retry only the noisy boundary, not the entire request.</li>
  <li>Spawn sibling work only when both tasks should die together.</li>
  <li>Map domain misses to explicit HTTP errors like <code>NotFoundError</code>.</li>
</ul>
